        ...
        <div className="mt-4 grid gap-4">
          ...
          <ResultSection title="Course status buckets">
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              <CourseCodeBucket title="Completed" codes={analysis.completedCourseCodes} />
              <CourseCodeBucket title="Preregistered" codes={analysis.preregisteredCourseCodes} />
              <CourseCodeBucket title="In progress" codes={analysis.inProgressCourseCodes} />
              <CourseCodeBucket title="AP/transfer satisfied" codes={analysis.transferOrApCourseCodes} />
              <CourseCodeBucket title="Fall Through/non-degree" codes={analysis.nonDegreeApplicableCourseCodes} />
              <CourseCodeBucket title="Still needed" codes={analysis.stillNeededCourseCodes} />
            </div>
          </ResultSection>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4 sm:p-5">
          <div className="grid gap-4">
            ...
            <ResultSection title="Parsed worksheet course statuses">
              <ParsedCurrentCourseStatuses records={analysis.courseStatusRecords} />
            </ResultSection>
...
function CourseCodeBucket({ title, codes }: { title: string; codes: string[] }) {
  return (
    <div className="rounded-md border border-slate-200 bg-slate-50 p-3">
      <p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-slate-500">{title}</p>
      <p className="mt-1 text-[18px] font-semibold leading-6 text-slate-950">{codes.length}</p>
      {codes.length > 0 ? (
        <div className="mt-2 flex flex-wrap gap-1.5">
          ...
          {codes.length > 8 ? <span className="text-[12px] text-slate-500">+{codes.length - 8} more</span> : null}
        </div>
      ) : null}
    </div>
  );
...
function ParsedCurrentCourseStatuses({
  ...
  return (
    <div className="grid gap-2">
      {records.map((record) => (
        <div className="rounded-md border border-slate-200 bg-slate-50 p-3" key={`${record.code}-${record.status}`}>
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-semibold text-slate-950">{record.code}</span>
